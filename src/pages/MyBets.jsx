  ...
  return (
    <div className="max-w-[75rem] mx-auto px-5 pb-5">
      ...
      <header className="text-center mb-10 pb-5 border-b-2 border-dark-border">
        <h1 className="text-2xl md:text-5xl font-bold mb-2.5 bg-gradient-to-r from-primary-500 to-secondary-500 bg-clip-text text-transparent pb-1">
          Mes Paris
        </h1>
      ...
      <div className="flex gap-3 mb-6 flex-wrap">
        {['all', 'pending', 'won', 'lost'].map(f => (
          ...
          >
            ...
            ({bets.filter(b => f === 'all' || b.status === f).length})
          </button>
        ))}
      </div>
      ...
      {filteredBets.length === 0 ? (
        <div className="text-center p-10 bg-dark-card rounded-xl border-2 border-dark-border">
          ...
          <p className="text-gray-400 text-lg mb-2.5">
            {filter === 'all' ? "Vous n'avez pas encore placé de paris" : `Aucun pari ${filter === 'pending' ? 'en attente' : filter === 'won' ? 'gagné' : 'perdu'}`}
          </p>
          {filter === 'all' && (
            <Link to="/" className="inline-block mt-4 px-6 py-3 bg-gradient-to-r from-primary-500 to-secondary-500 text-white rounded-lg font-bold hover:scale-105 transition-transform">
              Voir les matchs
            </Link>
          )}
        </div>
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <span className="text-xs text-gray-500">{formatDate(bet.created)}</span>
                    </div>
                    {match ? (
